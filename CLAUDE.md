...
`pre-session-scanner/scanner.py` has two hand-maintained lists worth remembering to revisit: `FOMC_MEETING_DATES` (the Fed publishes each year's schedule the prior year — refresh annually from federalreserve.gov) and `GEOPOLITICAL_KEYWORDS` (a static keyword list, matched against title *and* summary, used to flag a geopolitical-risk subset of the fetched news headlines — a heuristic, not a classifier, so it'll miss new terms as geopolitical themes shift and may need the odd addition). Note the latter has a structural ceiling beyond just keyword drift: it only scans headlines already fetched for the watchlist/oil/gold (not a dedicated search), so an empty result is often correct-but-uninformative — it means nothing surfaced in *those specific tickers'* feeds that day, not that there's no geopolitical risk anywhere. A real fix would be a dedicated feed (GDELT, already scoped as a Phase 2 source in the design doc), not more keywords.
