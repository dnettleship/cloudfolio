...
`pre-session-scanner/scanner.py` has two hand-maintained lists worth remembering to revisit: `FOMC_MEETING_DATES` (the Fed publishes each year's schedule the prior year — refresh annually from federalreserve.gov) and `GEOPOLITICAL_KEYWORDS` (a static keyword list used to flag a geopolitical-risk subset of the fetched news headlines — a heuristic, not a classifier, so it'll miss new terms as geopolitical themes shift over time and may need the odd addition).
