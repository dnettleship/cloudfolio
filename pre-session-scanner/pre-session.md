...
- `scanner.py` also fetches recent headlines (via yfinance's free `.news`
  property, scoped to the benchmark, VIX, oil and gold — not the full
  watchlist, to keep the yfinance call count bounded) and keeps a trailing
  90-session price history for oil/gold. Both are informational, like
  screener — not part of the publish gate. The report prompt may reference
  a headline by title; it never invents URLs, since links are rendered
  straight from the `news` field, not generated by Claude.
- On the Cloudfolio site (`infra/cloudfolio/frontend/index.html`): a
  **Pre-session Scanner** tab (the "Run scan" button, live report,
  headlines, regime stats, oil/gold price-trend charts, and a screener
  bar chart + table) and an **Archive** tab (`GET /archive`, lists every
  past scan saved to S3, click to expand the same report/headlines/charts
  for that scan). Charts are plain inline SVG built client-side from the
  JSON already in the response — no charting library, no server-side
  image generation. Nav order is Pre-session Scanner → Archive → Tracker.
