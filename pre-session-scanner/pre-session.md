...
- `scanner.py` also fetches recent headlines (via yfinance's free `.news`
  ...
  subset — a keyword match against `GEOPOLITICAL_KEYWORDS` (see CLAUDE.md
  for the note on keeping that list current), not a verified
  classification, surfaced since oil/gold are unusually geopolitically-
  driven (see Purpose in the design doc).
...
- On the Cloudfolio site (`infra/cloudfolio/frontend/index.html`): a
  **Pre-session Scanner** tab (the "Run scan" button, live report,
  headlines, geopolitical risk, upcoming events, regime stats, oil/gold
  price-trend charts, and a screener bar chart + table) and an **Archive**
