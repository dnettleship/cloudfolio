...
- `report.py` turns a scan's data into a short written report via Claude
  ...
  report is three parts: overall market conditions (not just the
  watchlist), an upcoming-events note, then a labeled "Likely near-term
  bias" call for the *broader market* — the watchlist screener is
  supporting color for that call, not its basis.
- `scanner.py` also fetches recent headlines (via yfinance's free `.news`
  ...
  for oil/gold. Both are informational, like screener — not part of the
  publish gate. The report prompt may reference a headline by title; it
  never invents URLs, since links are rendered straight from the `news`
  field, not generated by Claude.
- `scanner.py` also has a `calendar` dimension: near-term watchlist
  earnings dates (yfinance's free `.calendar`, per ticker) and the next
  FOMC meeting date (`FOMC_MEETING_DATES`, a small hardcoded list —
  update annually from federalreserve.gov, same low-maintenance approach
  as the EIA/OPEC+ timing notes in the design doc). Informational only,
...
- On the Cloudfolio site (`infra/cloudfolio/frontend/index.html`): a
  **Pre-session Scanner** tab (the "Run scan" button, live report,
  headlines, upcoming events, regime stats, oil/gold price-trend charts,
  and a screener bar chart + table) and an **Archive** tab (`GET
  /archive`, lists every past scan saved to S3, click to expand the same
  report/headlines/events/charts for that scan). Charts are plain inline
  SVG built client-side from the JSON already in the response — no
  charting library, no server-side image generation. Nav order is
  Pre-session Scanner → Archive → Tracker.
